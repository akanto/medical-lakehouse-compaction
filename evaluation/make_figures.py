...
L4 is not plotted. It tracks L3 to a median difference of 0.2% in W1, 0.3% in
W2 and 0.9% in W3, so the two lines would overprint; it stays in the tables,
...
1-2-5 ladder. Each workload keeps its own ticks and limits.

Each plotted value is the median of the three pass totals, as median_wall
describes.
...
# Every workload now keeps its own y scale. W1 and W2 shared one while a W1
# value was a single-series median and the two ranges overlapped (1.55 to
# 142.64 s against 6.40 to 127.19 s). Under the pass-total aggregation W1 runs
# 14.25 to 710.16 s, above W2 throughout, so a shared scale would push W2 into
# the lower third of its panels and invite a comparison of heights between two
# workloads of different size.
SHARED_Y = ()
...
def median_wall(w, rate, lat, s):
    """Median over the three passes of one cell's total wall-clock time.

    A pass is one end-to-end execution of the workload, so its total sums
    every run the workload comprises: the five series of W1, and the single
    execution of W2 and W3. Changed on 2026-09-17 from a median over the raw
    records, which for W1 mixed five series of different sizes with the three
    repetitions and reported the middle series rather than the workload. The
    three workloads are now aggregated identically, and each cell carries
    three comparable values whose dispersion is defined.
    """
    v = [r for r in rs
         if r["workload"] == w and r["rate_gbit"] == rate
         and r["latency_ms"] == lat and r["strategy"] == s]
    passes = sorted({r["run_index"] for r in v})
    return st.median([sum(r["wall_clock_s"] for r in v
                          if r["run_index"] == i) for i in passes])
