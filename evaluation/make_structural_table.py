...
def structural_stats(results: list) -> dict:
    """{(layout, workload): {n_req, rar, n}}, per workload execution.

    One execution is one pass of the workload in one cell, so the entries of
    a pass are summed before the median is taken: W1 reconstructs five series
    and its counters are the sum over them, while W2 and W3 contribute a
    single entry per pass and are unaffected. Changed on 2026-09-17, when the
    paper redefined a W1 value as the pass total; the earlier median over raw
    entries reported the middle of the five series instead of the workload.
    """
    passes = defaultdict(list)
    for entry in results:
        passes[(entry["strategy"], entry["workload"], entry["rate_gbit"],
                entry["latency_ms"], entry["run_index"])].append(entry)

    grouped = defaultdict(list)
    for key, entries in passes.items():
        wire = sum(e["bytes_on_wire"] for e in entries)
        useful = sum(e["bytes_useful"] for e in entries)
        grouped[(key[0], key[1])].append({
            "n_req": sum(e["s3_total_requests"] for e in entries),
            "rar": wire / useful if useful else None,
        })

    stats = {}
    for key, execs in grouped.items():
        rars = [e["rar"] for e in execs if e["rar"] is not None]
        stats[key] = {
            "n_req": statistics.median([e["n_req"] for e in execs]),
